...
def process_json_files(champion, build_file):
    ...
    with open(f"actions/u.gg/{champion}") as champion_file, open(build_file) as build:
        ...
        for i, items in enumerate(loaded_champion[0]["itemBuilds"][0]["blocks"]):
            try:
...
if __name__ == "__main__":
    for filename in os.listdir("actions/u.gg"):
        if filename.endswith(".json"):
            print(filename)
            modified_build = process_json_files(filename, "actions/build.json")
            write_json(modified_build, f"actions/u.gg/{filename}")